...
exports.cancelTrade = exports.acceptTrade = exports.proposeTrade = void 0;
...
exports.proposeTrade = functions.https.onCall(async (data, context) => {
    (0, appCheck_1.validateAppCheck)(context, "proposeTrade");
    ...
    const friendRef = firebase_1.db.collection(firebase_1.COLLECTIONS.USERS).doc(fromUid).collection(firebase_1.COLLECTIONS.FRIENDS).doc(toUid);
    ...
    const recentOffersSnap = await firebase_1.db
        .collection(firebase_1.COLLECTIONS.TRADE_OFFERS)
        .where("fromUid", "==", fromUid)
        .where("createdAt", ">=", oneDayAgo)
        .get();
    if (recentOffersSnap.size >= MAX_DAILY_PROPOSALS) {
        throw new functions.https.HttpsError("resource-exhausted", `Has alcanzado el límite de ${MAX_DAILY_PROPOSALS} ofertas de intercambio por día.`);
    }
    // 3. Obtener nombres públicos
    const [fromUserDoc, toUserDoc] = await Promise.all([
        firebase_1.db.collection(firebase_1.COLLECTIONS.USERS).doc(fromUid).get(),
        firebase_1.db.collection(firebase_1.COLLECTIONS.USERS).doc(toUid).get(),
    ]);
    ...
    const tradeRef = firebase_1.db.collection(firebase_1.COLLECTIONS.TRADE_OFFERS).doc();
    ...
    await tradeRef.set({
        ...
        expiresAt: firebase_1.Timestamp.fromDate(expiresAt),
    });
    ...
    try {
        await firebase_1.db.collection(firebase_1.COLLECTIONS.ANALYTICS_EVENTS).add({
...
exports.acceptTrade = functions.https.onCall(async (data, context) => {
    (0, appCheck_1.validateAppCheck)(context, "acceptTrade");
    ...
    const tradeRef = firebase_1.db.collection(firebase_1.COLLECTIONS.TRADE_OFFERS).doc(tradeId);
    return await firebase_1.db.runTransaction(async (transaction) => {
        ...
        if (trade.status !== "pendiente") {
            throw new functions.https.HttpsError("failed-precondition", `La oferta ya no está pendiente (Estado actual: ${trade.status}).`);
        }
        // 4. Validar caducidad
        if (trade.expiresAt && trade.expiresAt.toDate() < new Date()) {
            transaction.update(tradeRef, { status: "expirado", closedAt: firebase_1.FieldValue.serverTimestamp() });
        ...
        if (fromQty < offeredQty) {
            ...
            throw new functions.https.HttpsError("failed-precondition", "Intercambio cancelado: El proponente ya no posee la carta que ofreció.");
        }
        ...
        if (toQty < requestedQty) {
            ...
            throw new functions.https.HttpsError("failed-precondition", "Intercambio cancelado: Ya no posees la carta solicitada en tu inventario.");
        }
        // 6. TRANSFERENCIA ATÓMICA SIMULTÁNEA DE CARTAS
        const toReceivedCardRef = firebase_1.db
            .collection(firebase_1.COLLECTIONS.USERS)
            .doc(toUid)
            .collection(firebase_1.COLLECTIONS.USER_COLLECTION)
            .doc(offeredCardId);
        const fromReceivedCardRef = firebase_1.db
            .collection(firebase_1.COLLECTIONS.USERS)
            .doc(fromUid)
            .collection(firebase_1.COLLECTIONS.USER_COLLECTION)
            .doc(requestedCardId);
        ...
        if (fromQty <= offeredQty) {
            ...
            transaction.update(fromOfferedCardRef, { quantity: firebase_1.FieldValue.increment(-offeredQty) });
        }
        // Acreditar carta ofrecida a toUid
        if (toReceivedSnap.exists) {
            transaction.update(toReceivedCardRef, { quantity: firebase_1.FieldValue.increment(offeredQty) });
        }
        else {
        ...
        if (toQty <= requestedQty) {
            ...
            transaction.update(toRequestedCardRef, { quantity: firebase_1.FieldValue.increment(-requestedQty) });
        }
        // Acreditar carta solicitada a fromUid
        if (fromReceivedSnap.exists) {
            transaction.update(fromReceivedCardRef, { quantity: firebase_1.FieldValue.increment(requestedQty) });
        }
        else {
        ...
        const analyticsRef = firebase_1.db.collection(firebase_1.COLLECTIONS.ANALYTICS_EVENTS).doc();
...
exports.cancelTrade = functions.https.onCall(async (data, context) => {
    (0, appCheck_1.validateAppCheck)(context, "cancelTrade");
    ...
    const trade = tradeSnap.data();
    if (trade.status !== "pendiente") {
        throw new functions.https.HttpsError("failed-precondition", `La oferta ya no puede cancelarse (Estado actual: ${trade.status}).`);
    }
    let newStatus;
    let message;
